...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; } body { margin:0; color:var(--ink); background:var(--paper); font-family: Georgia, 'Times New Roman', serif; }
        ...
        .missing .status { background:var(--red); } .track strong { display:block; font-size:17px; font-weight:500; } .track small { color:var(--muted); font:13px Arial,sans-serif; }
...
<body><main>
    ...
    <div class="layout">
        <section>
            <div class="field"><label for="name">Nome da playlist</label><input id="name" value="Playlist Rap Tuga - Amor, Saudade & Madrugada"></div>
        ...
        <section><div class="results-head"><h2>Prévia</h2><div class="count" id="count">aguardando sua lista</div></div><div id="results" class="empty">Sua seleção aparecerá aqui.</div><button class="primary" id="create" disabled>Criar no Spotify</button><button class="secondary" id="login">Conectar Spotify</button></section>
    </div>
</main>
<script>
...
function render(data) { preview = data; const ok = data.adicionadas || []; const bad = data.naoEncontradas || []; $('count').textContent = `${ok.length} encontradas / ${bad.length} não encontradas`; $('results').className = ''; $('results').innerHTML = [...ok.map(t => `<div class="track"><span class="status">✓</span><div><strong>${escapeHtml(t.nome)}</strong><small>${escapeHtml(t.artista)}</small></div></div>`), ...bad.map(t => `<div class="track missing"><span class="status">×</span><div><strong>${escapeHtml(t)}</strong><small>Música não encontrada</small></div></div>`)].join('') || '<div class="empty">Nenhuma música válida foi encontrada.</div>'; $('create').disabled = !ok.length; $('create').textContent = $('existing').value.trim() ? 'Adicionar sem duplicar' : 'Criar no Spotify'; }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path not in ("/preview", "/create-playlist"):
